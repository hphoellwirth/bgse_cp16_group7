...
#Importing data
mydata = read.csv('concentration.csv',sep = ';',stringsAsFactors = FALSE,na.strings = 'NULL')
library(forecast)


#Calculating ARIMA models for each combination of station and pollutant
k = 0
first = 0
model = list()
predictions = list()
lab = matrix(NA,12610,2)
for (j in levels(as.factor(mydata$stationID))){
  for (i in levels(as.factor(mydata$pollutantID))){
    k = k + 1
    mydata1=mydata[mydata$stationID==j & mydata$pollutantID==i,]
    ...
    if (dim(mydata1)[1]>1){
      ...
      if (length(model[[k]])>0){
        ...
        station = matrix(j,length(prediction),1)
        pollutant = matrix(i,length(prediction),1)
        year = seq(max(mydata[mydata$stationID==j & mydata$pollutantID==i,'year'])+1,max(mydata[mydata$stationID==j & mydata$pollutantID==i,'year'])+5,1)
        low_95 = as.matrix(predictions[[k]]$lower[,2])
        up_95 = as.matrix(predictions[[k]]$upper[,2])
        temp = data.frame(station,pollutant,year,prediction,low_95,up_95)
        if (first==1){
